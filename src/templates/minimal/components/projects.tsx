...
interface Project { id?: string; title: string; description: string; tags: string[]; github?: string; live?: string; featured?: boolean; }
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <AnimatePresence>
            {displayed.map((project, i) => (
              <motion.div key={project.id ?? project.title} layout
                initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.97 }} transition={{ duration: 0.35, delay: i * 0.06 }}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {project.github && <a href={project.github} target="_blank" rel="noopener noreferrer" className="text-[#52525b] hover:text-[#fafafa] transition-colors"><Github className="w-4 h-4" /></a>}
                    {project.live && <a href={project.live} target="_blank" rel="noopener noreferrer" className="text-[#52525b] hover:text-[#3b82f6] transition-colors"><ExternalLink className="w-4 h-4" /></a>}
                  </div>
                </div>
                <div className="flex-1">
                  <h3 className="text-sm font-semibold text-[#fafafa] mb-2 group-hover:text-[#3b82f6] transition-colors">{project.title}</h3>
                  <p className="text-xs text-[#71717a] leading-relaxed">{project.description}</p>
                </div>
        ...
        {rest.length > 0 && (
          <div className="mt-8">
            <button onClick={() => setShowAll((v) => !v)}
              ...
              {showAll ? <><ChevronUp className="w-4 h-4" /> Tampilkan lebih sedikit</> : <><ChevronDown className="w-4 h-4" /> Tampilkan {rest.length} proyek lagi</>}
            </button>
          </div>
        )}
      </div>
